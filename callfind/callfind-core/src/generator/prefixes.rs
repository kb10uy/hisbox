...
pub type PrefixData = (Vec<(Prefix, usize)>, Vec<String>);

pub fn load_prefixes(path: &str) -> IoResult<PrefixData> {
...
pub fn write_prefixes<W: Write>(
    writer: &mut W,
    prefixes: &[(Prefix, usize)],
    area_names: &[String],
) -> IoResult<()> {
    writeln!(writer, r#"pub const PREFIXES: &[(Prefix, usize)] = &["#)?;
    for (p, i) in prefixes {
        let prefix_str = match p {
            Prefix::OneAll(p1) => format!("Prefix::OneAll({p1})"),
            Prefix::TwoRange(p1, (p2s, p2e)) => {
                format!("Prefix::TwoRange({p1}, ({p2s}, {p2e}))")
            }
            Prefix::TwoSpecified(p1, p2) => {
                format!("Prefix::TwoSpecified({p1}, {p2})")
            }
            Prefix::ThreeRange(p1, p2, (p3s, p3e)) => {
                format!("Prefix::ThreeRange({p1}, {p2}, ({p3s}, {p3e}))")
            }
        };
        writeln!(writer, r#"    ({prefix_str}, {i}),"#)?;
    }
    writeln!(writer, r#"];"#)?;

    writeln!(writer, r#"pub const AREA_NAMES: &[&str] = &["#)?;
    for name in area_names {
        writeln!(writer, r#"    "{}","#, name.escape_default())?;
    }
    writeln!(writer, r#"];"#)?;
    writeln!(writer)?;
    Ok(())
